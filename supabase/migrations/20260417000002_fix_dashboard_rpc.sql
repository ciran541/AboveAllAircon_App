-- ─────────────────────────────────────────────────────────────────────────────
-- Fix dashboard RPC: Security + Stage Semantics
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  -- ── Security Check ──────────────────────────────────────────────────────────
  -- Enforce that only admins can successfully execute this function, despite the GRANT.
  SELECT role INTO v_user_role FROM public.profiles WHERE id = auth.uid();
  IF v_user_role IS DISTINCT FROM 'admin' THEN
    RAISE EXCEPTION 'Unauthorized: Only admins can view dashboard metrics.';
  END IF;
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE stage IN ('New Enquiry', 'Site Visit Scheduled') AND (status IS NULL OR status = 'open'))
